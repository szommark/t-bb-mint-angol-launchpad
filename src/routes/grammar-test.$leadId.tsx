...
const t = {
  en: {
    title: "Free Grammar Test",
    sub: "10 quick grammar questions that adapt to your level. About 5 minutes.",
    intake: {
  ...
  hu: {
    title: "Ingyenes nyelvtani teszt",
    sub: "10 gyors nyelvtani kérdés, amely alkalmazkodik a szintedhez. Kb. 5 perc.",
    intake: {
      heading: "Készen állsz?",
      start: "Teszt indítása",
    },
    generating: "Teszt előkészítése…",
  ...
  de: {
    title: "Kostenloser Grammatiktest",
    sub: "10 kurze Grammatikfragen, die sich deinem Niveau anpassen. Etwa 5 Minuten.",
    intake: {
      heading: "Bereit?",
      start: "Test starten",
    },
    generating: "Test wird vorbereitet…",
...
const deadlineKey = (leadId: string) => `grammar-deadline:${leadId}`;

export const Route = createFileRoute("/grammar-test/$leadId")({
  head: () => ({
    meta: [{ title: "Grammar Test — Több mint angol" }, { name: "robots", content: "noindex" }],
  }),
...
function GrammarTest() {
  ...
  const sessionToken = (() => {
    ...
    try {
      return sessionStorage.getItem(`lead-token:${leadId}`) ?? "";
    } catch {
      return "";
    }
  })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/public/grammar/state?leadId=${leadId}`, {
          headers: authHeaders,
        });
        if (!res.ok) {
          if (!cancelled) {
        ...
        if (cancelled) return;
        if (data.language && (["en", "hu", "de"] as const).includes(data.language))
  ...
  const startTest = async () => {
    setStep("generating");
    try {
      const res = await fetch("/api/public/grammar/start", {
        method: "POST",
      ...
      setSelected(null);
      try {
        localStorage.removeItem(deadlineKey(leadId));
      } catch {
        /* noop */
      }
      setStep("test");
  ...
  const submitTest = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/public/grammar/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...authHeaders },
        body: JSON.stringify({ leadId }),
      });
      ...
      <header className="border-b border-border/60 bg-background/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-3xl items-center justify-between px-5">
          <Link to="/" className="flex items-center gap-2">
            <span className="flex h-8 w-8 items-center justify-center rounded-md bg-[var(--gradient-hero)] text-xs font-bold text-primary-foreground shadow-[var(--shadow-card)]">
              T
            </span>
            ...
            return (
              <div className="rounded-3xl border border-border bg-card p-7 shadow-[var(--shadow-card)] sm:p-9">
                <div className="mb-6 flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                  <span>
                    {lc.test.q} {displayIdx} / {totalPlanned}
                  </span>
            ...
            <div className="rounded-3xl border border-border bg-card p-8 text-center shadow-[var(--shadow-elegant)] sm:p-12">
              ...
              {(() => {
                const rows = LEVELS.map((l) => ({
                  level: l,
                  ...(result.byLevel?.[l] ?? { correct: 0, total: 0 }),
                })).filter((r) => r.total > 0);
